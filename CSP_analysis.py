...
def setup_logging(no_log: bool, log: str):
    ...
    logger.setLevel(logging.WARNING if no_log else logging.INFO)
...
def get_consistent_max_residue_index(peaklists: List[pd.DataFrame], logger):
    max_residue_indices = []
    ...
    if len(max_residue_indices) == 1:
        ...
        return list(max_residue_indices)[0]
    else:
        logger.info(
...
def main():
    ...
    logger = setup_logging(args.no_log, f"{args.dir}/{args.log}")
    logging.info("You ran: " + str(sys.executable) + " " + " ".join(sys.argv))
    ...
    peakfiles = [peakfile for peakfile in peakfiles if peakfile.strip() != ""]
    logging.info(f"Using peakfiles:\n  {'\n  '.join(peakfiles)}")
    ...
    for nucleus in unique_nonH_nuclei:
        alphas[nucleus] = float(
            input(f"Give the alpha for scaling perturbations in {nucleus}: ").strip()
        )
        alphas[nucleus] = 0.2
    ...
    max_residue_index = get_consistent_max_residue_index(peaklists, logger)
    if max_residue_index:
        while True:
            autoscale = (
                input(
                    "Would you like to autoscale this residue's CSP to 1 in each histogram? (y/n): "
                )
                .lower()
                .strip()
            )
    ...
    for nucleus in unique_nonH_nuclei:
        for i, peaklist in enumerate(peaklists[1:]): # We do [1:] because first peaklist is baseline and the CSPs=0
            fig = plt.figure(figsize=(16,9))
            plt.title(f"H{nucleus} at {titration_percs[i]}%")
            ...
            plt.bar(peaks_filtered.ResidueIndex.to_numpy(dtype=int), CSPs_np, color=colors, width=1.0)
            plt.axhline(y=mean, color="black", linestyle='-', label=f"Mean: {mean:0.3f}")
            plt.axhline(y=mean+stdev, color="black", linestyle='--', label=f"Stdev: {stdev:0.3f}")
            plt.axhline(y=mean-stdev, color="black", linestyle='--',)
            plt.xticks(peaks_filtered.ResidueIndex.to_numpy(dtype=int), labels=peaks_filtered["Assignment"].str.split("_").str[0].tolist(), fontsize=8, rotation=45)
            plt.xlim(xmin=peaks_filtered.ResidueIndex.to_numpy(dtype=int).min()-0.5, xmax=peaks_filtered.ResidueIndex.to_numpy(dtype=int).max()+0.5)
            plt.xlabel("Residues")
            plt.ylabel(r"$\Delta \delta$" + " (ppm)")
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"{args.dir}/H{nucleus}_{titration_percs[i+1]}.png")
